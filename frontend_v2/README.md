...
>   offline dev)
>
> **Phase 5** (reveal / diff view) is a placeholder `reviewing` step for now.
...
Dev server runs on **http://localhost:3001** so it doesn't collide with the
...
│   └── api.ts                   # startPipeline / uploadResume + bearer token
├── types/
...
├── shell/
│   ├── AppShell.tsx             # Gradient canvas · header · context strip · feed · composer
...
└── conversation/
    ...
    ├── ConversationFeed.tsx     # Active-turn stage (swaps to ProcessingStream in Phase 4)
    ...
    ├── Composer.tsx             # Pill input + contextual UI slot (locks during PROCESSING)
    ├── ProcessingStream.tsx     # Phase 4 live stage — stacks step messages + aura
...
That's it — no routing or component wiring needed.

## Styling notes

- Tailwind v4 CSS-first config lives entirely in `src/index.css` via
  `@theme`; there's no `tailwind.config.js`.
